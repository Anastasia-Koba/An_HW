  ...
  render({title, description}) {
      ...
      <header>
          <div className="card card_header">
              ...
              <p className="card__title" title="${title}">${title}</p>
              <span className="card__description" title="${description}">${description}</span>
          </div>
      </header>`;
  }
}
...
class Person extends React.Component {
  ...
  render(options, {item}) {
      return `<div className="card card_person">
          <img className="card__img card__img_round" src="${item.photo || 'img/ui/default_pix.jpg'}" alt="Аватар ${item.title}" />
          <p className="card__title" title="${item.title || ''}">${item.title || ''}</p>
          <span className="card__description" title="${item.study || ''}">${item.study || ''}</span>
      </div>`;
  }
  ...
  openPersonPopup(item) {
      ...
      this.openPopupAction.execute({
          ...
          content: `<center><img height="300" width="300" className="card__img" src="${item.photo || 'img/ui/default_pix.jpg'}" alt="Аватар ${item.title}" /></center>`,
          offset: {
              left: 0,
              right: 0,
              top: 0,
...
*/
class PopupStack extends React.Component {
constructor(options) {
  super(options);
  this.popups = [];
}
...
class DataSet{
  ...
  list(){
      return this.query(
          ...
          .then(result =>{
             return result.map((result) => this.toModel(result));
          });
